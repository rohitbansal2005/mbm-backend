...
router.get('/followers/:userId', auth, async (req, res) => {
    try {
        ...
        .populate('follower', 'username fullName profilePicture role isPremium badgeType')
        .sort({ createdAt: -1 });
...
router.get('/following/:userId', auth, async (req, res) => {
    try {
        ...
        .populate('following', 'username fullName profilePicture role isPremium badgeType')
        .sort({ createdAt: -1 });
